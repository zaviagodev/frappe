frappe.ui.form.ControlConnection = class ControlConnection extends frappe.ui.form.ControlData {
	...
	make_input() {
		...
		// label
		this.label_span = $(`<span class="link-field-label">${this.df.label}</span>`).appendTo(
			this.$labelHolder
		);
		// count & link
		...
		// add button
		this.$button =
			$(`<button class="btn btn-new btn-secondary btn-xs icon-btn" data-doctype="">
