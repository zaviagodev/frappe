frappe.ui.form.ControlAttachMultipleImages = class ControlAttachMultipleImages extends (
			...
			frappe.ui.form.on(this.frm.doctype, {
				onload: () => {
					frappe.db
						...
						.then((attachments) => {
							const extra_attachments = attachments.filter(
								(attachment) =>
									!this.imageGrid.images.find(
										(img) => img.image === attachment.name
									)
							);
				...
				after_save: () => {
					const missing_attachments = this.imageGrid.images.filter(
						(img) =>
							!this.frm.attachments
								.get_attachments()
								.find((attachment) => attachment.name === img.image)
					);
	...
	set_value(attachment) {
		...
		this.imageGrid.pending_delete = this.imageGrid.pending_delete.filter(
			(img) => img.image !== attachment.image
		);
	}
	...
	async on_upload_complete(attachment) {
		...
		if (this.frm) {
			this.frm.set_value(this.df.fieldname, [...this.imageGrid.images, newImage]);
		}
	...
	make() {
		...
		new Sortable(this.wrapper[0], {
			...
			onEnd: (e) => {
				let newSort = [];
				e.to
					.querySelectorAll(".grid-image")
					.forEach((img) => newSort.push(img.getAttribute("image")));
