	...
	var save = function () {
		...
		if ((action !== "Save" || frm.is_dirty()) && check_mandatory()) {
			...
			if(frm.doctype == 'Sales Invoice'){
				if(frm.doc.__islocal || action === "Submit"){
					var headers = $('.apps-nav-bar-top');
					...
					if (headers.length > 0) {
						...
						const order_in_progress = $(".save_state .first_action h3")
						const order_totals = $(".order_totals")

						$(".navbar").addClass("is-sales-invoice-creating")
						...
						gsap.set(headers, {maxWidth:"20px", opacity: 0})
						gsap.set(order_in_progress, {filter:"blur(5px)",opacity:0,x:20})
						gsap.set(order_totals, {filter:"blur(5px)"})

						gsap.to(headers, {duration:0.75, maxWidth:"526px",ease: "power2.out"})
						setTimeout(() => {
							gsap.to(headers, {duration:0.5, opacity: 1, ease: "power2.out"})
						}, 250)
						setTimeout(() => {
							gsap.to(order_in_progress, {duration:1, opacity: 1, filter:"blur(0px)",x:0, ease: "power2.out"})
						}, 500)

						setTimeout(() => { 
							gsap.to(third_action, {duration:0.5,height: "auto", opacity: 1, y: -60,x: 0,ease: "power2.out", filter:"blur(0px)",transformOrigin:"0"});
							gsap.to(third_action, {duration:1,ease: "elastic.out(1.5, 1)",scale:1});
							...
							gsap.set($('.save_state .third_action .images_list .image_f'), {opacity: frm.doc.is_return ? 1 : 0})

							let images = $(".save_state .third_action .images_list .image_f")
							images.each(function(index) {
								setTimeout(() => {
									$(this).css({ opacity: frm.doc.is_return ? 0 : 1, transition: "opacity 500ms", transitionDelay: frm.doc.is_return ? "100ms" : "0ms" });
								}, (2000 / images.length) * index);
							});
						}, 3000);
						setTimeout(() => { 
							gsap.to(third_action, {duration:1,height: frm.doc.is_return ? "0px" : "auto", scale:1,ease: "elastic.out(1.5, 1)"});
							gsap.to(second_action, {duration: 1.5,height: "80px", opacity: 1, y: -20, ease: "elastic.out(1.5, 1)"});
							gsap.to(order_totals, {duration:1,filter:"blur(0px)",ease: "power2.out"})
						}, 5000);
						setTimeout(() => { 
							gsap.to(first_action, {duration:0.5, opacity: 0, y: 0, ease: "power2.out", filter:"blur(5px)"});
						...
						setTimeout(() => {
							...
							$(first_action).html(`<svg width="50" height="50" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
								...
								<div>
									<h3>${action == "Submit" ? (frm.doc.is_return ? "Refund order submitted" : "Order submitted") : (frm.doc.is_return ? "Refund order created" : "Order created")}</h3>
									<p>${frm.doc.name}</p>
								</div>`)
							gsap.to(first_action, {duration:0.5, opacity: 1, y: 0, ease: "power2.out", filter:"blur(0px)"});
						}, 7500)
					}
				}
			}
			...
			if(frm.doctype == 'Item'){
				if(frm.doc.__islocal || action === "Submit"){
					...
					if (headers.length > 0) {
						...
						var svgimgs = '<svg width="30" height="30" viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg"> <rect width="38" height="38" rx="19" fill="black"/> <circle opacity="0.4" cx="19" cy="19" r="12.047" stroke="#37C058" stroke-width="1.90596"/> <path d="M19.7049 6.91999C22.0666 7.12661 24.314 8.02892 26.1629 9.51281C28.0118 10.9967 29.3791 12.9955 30.092 15.2565C30.8049 17.5175 30.8313 19.9392 30.1679 22.2152C29.5045 24.4912 28.1811 26.5194 26.365 28.0432C24.5489 29.5671 22.3218 30.5182 19.9651 30.7763C17.6085 31.0344 15.2282 30.5879 13.1254 29.4932C11.0225 28.3985 9.29151 26.7049 8.15121 24.6264C7.01092 22.5479 6.51258 20.178 6.7192 17.8163" stroke="#37C058" stroke-width="2.5" stroke-linecap="round"/> </svg>';
						var structure = '<div class="save_state"><div class="first_action"><div class="icon-creating">' + creatingIcon + '<h3>Creating item</h3></div><span>'+svgimgs+'</span></div>';
						headers.html(structure);
						$("#body").append(loadingFreeze)
						$('body').css("overflow", "hidden")
			...
			setTimeout(function() { 
				_call({
					...
					callback: function (r) {
							if(frm.doctype == 'Sales Invoice' || frm.doctype == 'Item'){
								var headers = $('.apps-nav-bar-top');
								$("#freeze-save").remove()
							...
							$(".navbar").removeClass("is-sales-invoice-creating")
